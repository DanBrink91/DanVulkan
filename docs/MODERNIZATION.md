...
- Seventeen tests now run: eleven CPU-focused asset, animation, device, descriptor, swapchain, graphics-pipeline, memory-policy, lighting-planning, environment-precomputation, scene-planning, and scene-context paths, plus six renderer validation/stress paths.
...
- `SceneSubmission` makes the per-frame view, projection, camera position, bounded point-light list, and environment controls explicit rather than sourcing them unconditionally from the renderer's demo camera.
...
- Vulkan-free `ScenePointLight` and `SceneEnvironment` descriptions carry per-frame light position, range, color, and intensity plus environment tint, intensity, rotation, diffuse strength, and specular strength. A pure planner validates finite, nonnegative values and the configured capacity before packing the GPU ABI into retained-capacity scratch.
...
- The PBR shader accumulates Cook-Torrance direct lighting for all submitted point lights with inverse-square attenuation and optional smooth range falloff. Environment rotation and tint apply consistently to irradiance and reflection lookups; the specular path now uses the roughness-prefiltered map and split-sum BRDF response rather than an ad hoc roughness multiplier.
- The descriptor planner permanently reserves three environment samplers separately from the bindless material array. Point-light storage occupies binding 7; irradiance, prefiltered specular, and BRDF sampling occupy bindings 8 through 10 without coupling lighting resources to imported scene ownership.
- Environment precomputation currently runs synchronously on the CPU during renderer initialization with bounded output sizes and 32 deterministic integration samples. Disk caching or GPU compute precomputation is a later startup-quality optimization. Shadows and `KHR_texture_transform` also remain future rendering work.
...
- The demo composes `ninja_run_free_fire_emote.glb` into the village at an authored scale and loops its clip automatically after each submitted camera is available, including host-owned application loops.
...
1. **Directional/spot lighting and shadows** - introduce bounded directional and spot-light descriptions, GPU buffers, and shader evaluation, then add shadow-map resources without folding those lifetimes back into `renderer.cpp`.
...
The recommended next implementation slice adds bounded directional and spot lights behind the existing lighting planner/context boundary. Keeping that work shadow-independent first will settle the public light ABI and shader evaluation before shadow passes introduce frame-graph, depth-image, and synchronization requirements.
