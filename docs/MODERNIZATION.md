...
- Nineteen tests now run: twelve CPU-focused application-camera, asset, animation, device, descriptor, swapchain, graphics-pipeline, memory-policy, lighting-planning, environment-precomputation, scene-planning, and scene-context paths, plus seven renderer validation/stress paths.
